...
function calcularData() {
    ...
    resultadoDivEl.classList.add('opacity-0');
    infoLegalDiv.classList.add('hidden');
    resultadoDivEl.classList.remove('bg-green-100', 'text-green-800', 'bg-red-100', 'text-red-800', 'bg-yellow-100', 'text-yellow-800');
    ...
    // Função interna para tocar o som de forma segura
    const tocarSom = () => {
      somDinheiroEl.currentTime = 0; // Garante que o som comece do início
      const promise = somDinheiroEl.play();
      if (promise !== undefined) {
        promise.catch(error => {
          console.error("A reprodução de áudio foi bloqueada pelo navegador.", error);
        });
      }
    };
